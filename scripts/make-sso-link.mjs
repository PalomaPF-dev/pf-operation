 ...
 * マスタを編集したいときは --department に MASTER_EDIT_DEPARTMENTS（既定「生産管理部」）を渡すか、
 * --portal-admin を付ける（ポータル管理者は部署によらず編集できる）。
 * ※ ポータル登録後は、このスクリプトは不要。
 */
...
// ポータル管理権限。付けると部署によらずマスタを編集できる（--portal-admin）
const portalAdmin = process.argv.includes("--portal-admin");
