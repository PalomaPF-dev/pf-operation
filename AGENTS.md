...
- 利用者の区別はすべてポータルのログイン情報で行う（アプリ側の利用者マスタは持たない）。
  閲覧・入力できる工場は、ポータル連携の所属（工場名・部署名）とマスタの工場名の一致で絞る（`getUserScope`）。
  **役割では緩めない**（所属工場が基準）: 工場に所属する人は生産管理部・ポータル管理者であっても自工場のみ。
  工場に所属しない人（部署が工場でない）だけが全工場を閲覧・入力できる。
  画面の絞り込みだけでなく、`saveReportAction` でも `isLineInScope` で必ず検証する。
  ダッシュボード・報告履歴・集計・CSV出力にも同じスコープを適用する（`scopeFactoryIds` を各読み取りへ渡す）。
